...
class Linear_Config(SequentialModelConfig):
    ...
    def get_build(self):
        build = self.layerbuild(["block_cls", "in_feature", "hidden_dim", None, "act_cls", "true"])
        build += self.layerbuild(["block_cls", "hidden_dim", "hidden_dim", "norm_cls", "act_cls"], self.get("n_blocks"))
        build += self.layerbuild(["block_cls", "hidden_dim", "out_dim", None, None, "true"])
        self.build = build
...
class RNN_Config(SequentialModelConfig):
    ...
    def get_build(self):
        build = self.layerbuild(["linear_cls", "in_feature", "hidden_dim", None, "act_cls", "true"])
        build += self.layerbuild(["rnn_cls", "hidden_dim", "hidden_dim", "n_layers", "bias", "batch_first", "dropout"])
        build += self.layerbuild(["linear_cls", "hidden_dim", "out_dim", None, None, "true"])
