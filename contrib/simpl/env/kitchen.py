...
class KitchenEnvOT(KitchenEnv):
    """
    Environment giving penalty on performing out of target task.
    """
    def __init__(self, *args, **kwargs):
        self.TASK_ELEMENTS = ['bottom burner', 'top burner', 'light switch', 'slide cabinet', 'hinge cabinet', 'microwave', 'kettle']
        self.set_non_target_task()
        super().__init__(*args, **kwargs)

    def set_non_target_task(self):
        self.non_target_task = [t  for t in all_tasks if t not in self.TASK_ELEMENTS]
    ...
    def compute_reward(self, obs_dict):
        ...
        # non-target task에 대해 달성 시
        # 0 reward 
        # 끝내야 함.
        # 지속하는 경우 : 나머진 다 했지만 중간에 하나 껴서 reward를 못받음. 잘못학습

        for element in self.non_target_task:
            element_idx = OBS_ELEMENT_INDICES[element]
            distance = np.linalg.norm(
                next_obj_obs[..., element_idx - idx_offset] -
                next_goal[element_idx])
