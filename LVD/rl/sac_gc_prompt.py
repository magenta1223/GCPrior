...
class SAC(BaseModule):
    def __init__(self, config):
        ...
        self.policy_optim = torch.optim.Adam(
            [
                # { "params" : self.policy.inverse_dynamics.subgoal_generator.parameters(), "lr" : self.policy_lr}, # 보상최대화하는 subgoal 뽑기. 
                { "params" : self.policy.goal_prompt,  "lr" : self.policy_lr}, # 보상최대화하는 subgoal 뽑기. 
            ], # 낮추면 잘 안됨. 왜? 
        )
    ...
    def update_subgoal_generator(self, step_inputs):
        results = {}
        dist_out = self.policy.dist(step_inputs, "finetune_f")
