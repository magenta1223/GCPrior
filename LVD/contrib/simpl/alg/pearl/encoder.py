...
class StochasticEncoder(nn.Module):
    ...
    def encode(self, list_batch, sample):
        ...
        if len(list_batch) == 0:
            ...
            # sample할거면 임의의 latent vector를 생성하고
            if sample is True:
                z = dist.sample()
            # 아니면 그냥 평균 갖고옴
            else:
            ...
            # 뭔가 입력이 있다. batch를 MDP의 transition형태로 변경 후 matrix로 만듬
            transitions = torch.cat([batch.as_transitions() for batch in list_batch], dim=0)
            with torch.no_grad():
                # task encoder를 통해 task embedding의 분포를 얻는다. 
...
class SetTransformerEncoder(StochasticEncoder):
    ...
    def dist(self, batch_transitions):
        # set transformer의 출력을 time dimension에 대해 쪼개서
        # loc (분포의 평균)과 pre_scale(분산)로 사용. 
