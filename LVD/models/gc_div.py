...
class GoalConditioned_Diversity_Model(BaseModule):
    """
    """
    def __init__(self, model_config):
        ...
        if not self.joint_learn:
            ...
            # target_state_encoder = ae.target_state_encoder.eval()

            state_encoder.requires_grad_(False)
            state_decoder.requires_grad_(False)
            # target_state_encoder.requires_grad_(False)

            self.latent_state_dim = ae.latent_dim
        ...
        prior_wrapper_cls = PRIOR_WRAPPERS['gc_div']
    ...
    def get_metrics(self):
        ...
        with torch.no_grad():
            ...
            #     i = 0
                ...
                # task = GOAL_CHECKERS['kitchen'](self.loss_dict['states_novel'][0][-1].detach().cpu().numpy()[:30])
                ...
                #     text = f"{task} S-A now {i} c {self.c}" if self.c != 0 else f"S-A now {i}"
                ...
                # task = GOAL_CHECKERS['kitchen'](self.outputs['subgoal_recon_f'][i].detach().cpu().numpy()[:30])
