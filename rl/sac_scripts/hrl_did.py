...
from proposed.rl.sac_modules.sgspirl import SAC
...
from proposed.collector.sgspirl import LowFixedHierarchicalTimeLimitCollector
...
def simpl_fine_tune_iter(collector, trainer, batch_size, reuse_rate, task, G_final):
    log = {}

    # collect
    with trainer.policy.expl(), collector.low_actor.expl(), collector.env.step_render():
        episode = collector.collect_episode(trainer.policy, G_final)
    ...
    for i in range(max(n_step, 1)):
        stat = trainer.step(batch_size, G_final)
...
def train_single_task(env, task, args):
    task_obj = KitchenTask(subtasks = [task])
    # gpu = 0
    ...
    policy = MMPHSG(Linear_Config(policy_config), model.skill_prior, model.state_encoder, model.subgoal_generator)        
    ...
    G_final = init_state[39:]

    sac_config = {'auto_alpha': True, 'kl_clip': 20, 'target_kl': 5, 'increasing_alpha': True}
    self = SAC(policy, model.skill_prior, model.state_encoder, model.subgoal_generator, qfs, buffer, **sac_config)
    self = self.cuda()
    config = {'batch_size': 256, 'reuse_rate': 256, 'task' : task, "G_final" : G_final}
    ...
    with env.set_task(task_obj):
        ...
        for episode_i in range(n_episode+1):
            ...
            if (episode_i + 1) % 5 == 0:
                ...
                # print(sr)
                imgs = render_task(env, self.policy, low_actor, G_final)
                visualize(imgs = imgs, task_name = task)
            wandb.log(log)
        wandb.finish()    
